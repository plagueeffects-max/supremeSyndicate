      ...
      <div key={message.id} className={`flex flex-col ${isUser ? 'items-end' : 'items-start'} animate-message-in`}>
        ...
        {message.type === 'ai' &&
          ...
          getPresetOptions(nextExpectedField).length > 0 && (
            <div className="mt-3 ml-[56px] flex flex-wrap gap-2">
            ...
            <div ref={chatContainerRef} className="flex-1 h-full min-h-0 overflow-y-auto px-4 md:px-8 pt-6 pb-36 relative z-10">
              <div className="max-w-4xl mx-auto space-y-6">
                ...
                {chatPhase === 'ADVISOR' && !isSubmitting && !nextExpectedField && getAdvisorChips().length > 0 && (
                  <div className="flex flex-wrap gap-2 ml-16 animate-message-in">
                    {getAdvisorChips().map((chip, i) => (
                      <button
                        key={i}
                        type="button"
                        onClick={() => submitMessage(chip)}
                        className="px-4 py-2 glass-surface hover:bg-white/80 dark:hover:bg-gray-700/80 text-gray-700 dark:text-gray-300 rounded-full text-sm font-medium transition-all hover:shadow-sm hover:scale-[1.02] active:scale-95"
                      >
                        {chip}
                      </button>
                    ))}
